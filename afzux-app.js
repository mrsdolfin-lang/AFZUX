...
function calcCost(ver,dur){
  ...
  // All available AI models with speed data
  var AI_MODELS = [
    // GROQ models
    {id:'groq_llama33_70b', api:'groq', name:'Groq — LLaMA 3.3 70B', speed:300, speedLabel:'~300 t/s', quality:'⭐⭐⭐⭐', limit:'14,400/day', badge:'FAST', color:'#10B981', desc:'Best balance of speed and quality'},
    {id:'groq_llama31_8b',  api:'groq', name:'Groq — LLaMA 3.1 8B',  speed:500, speedLabel:'~500 t/s', quality:'⭐⭐⭐',   limit:'14,400/day', badge:'FASTEST', color:'#06B6D4', desc:'Fastest Groq model, simpler tasks'},
    // CEREBRAS models
    {id:'cerebras_llama4scout', api:'cerebras', name:'Cerebras — Llama 4 Scout', speed:2600, speedLabel:'~2,600 t/s', quality:'⭐⭐⭐⭐', limit:'~2,000/day', badge:'ULTRA', color:'#F59E0B', desc:'World\'s fastest AI — newest model'},
    {id:'cerebras_qwen3_32b',   api:'cerebras', name:'Cerebras — Qwen3 32B',     speed:1400, speedLabel:'~1,400 t/s', quality:'⭐⭐⭐⭐⭐', limit:'~2,000/day', badge:'SMART', color:'#8B5CF6', desc:'Best quality on Cerebras'},
    {id:'cerebras_llama31_8b',  api:'cerebras', name:'Cerebras — LLaMA 3.1 8B', speed:1800, speedLabel:'~1,800 t/s', quality:'⭐⭐⭐',   limit:'~2,000/day', badge:'QUICK', color:'#EC4899', desc:'Fastest small model'},
    // GEMINI models
    {id:'gemini_flash_lite', api:'gemini', name:'Gemini 2.5 Flash-Lite', speed:263, speedLabel:'~263 t/s', quality:'⭐⭐⭐⭐', limit:'1,000/day', badge:'FREE', color:'#3B82F6', desc:'Best free daily limit'},
  ];
...
function AFZUXApp(){
  ...
  var s60=useState({
    groqKeys:[''],
    cerebrasKeys:[''],geminiKeys:[''],
    geminiKeys:[''],
    hfToken:'',geminiKey:'',claudeKey:'',openaiKey:'',grokKey:''
  });
  var s_mc=useState({
    slowModel:'groq_llama33_70b',
    fastModel:'cerebras_llama4scout',
    slowFallback:'gemini_flash_lite',
    fastFallback:'groq_llama33_70b'
  }); var modelConfig=s_mc[0]; var setModelConfig=s_mc[1]; var aiKeys=s60[0]; var setAiKeys=s60[1];
  // Generate tab
  ...
  var MUTED='#606080'; var FONT_FAMILY="'Inter',sans-serif";
var GOLD='#C9A84C'; var GREEN='#2ECC8A';
  ...
  var s_usage = useState({groqUsed:0,cerebrasUsed:0,date:''}); 
  var usage = s_usage[0]; var setUsage = s_usage[1];

  function loadUsage(){
    var today = new Date().toISOString().slice(0,10);
    var gKey = 'sh_fizux:usage:groq:'+today;
    var cKey = 'sh_fizux:usage:cerebras:'+today;
    var gUsed = 0; var cUsed = 0;
    try{ gUsed = parseInt(localStorage.getItem(gKey)||'0'); }catch(e){}
    try{ cUsed = parseInt(localStorage.getItem(cKey)||'0'); }catch(e){}
    setUsage({groqUsed:gUsed, cerebrasUsed:cUsed, date:today});
  }
  ...
  var filteredUsers=allUsers.filter(function(u){
    ...
    if(userFilter==='guest') return u.method==='guest';
  ...
  return h('div',{style:{minHeight:'100vh',background:BG,color:TEXT}},
    ...
    h('div',{style:{padding:'14px',maxWidth:'760px',margin:'0 auto'}},
      ...
      tab==='overview' && h('div',null,
        ...
        h('div',{style:{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'8px',marginBottom:'14px'}},
          [{icon:'👤',label:'Guest',val:stats.gu,color:BLUE},
        ...
        h('div',{style:card({marginBottom:'12px'})},
          h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}},
            h('div',{style:{fontWeight:700,fontSize:'13px',color:TEXT}},'⚡ API Keys Status'),
            h('button',{onClick:function(){setTab('aikeys');},style:{fontSize:'11px',color:GOLD,background:'none',border:'none',cursor:'pointer',fontFamily:'inherit'}},'Manage →')
          ),
          h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}},
            h('div',{style:{background:BG,borderRadius:'10px',padding:'10px',textAlign:'center'}},
              h('div',{style:{fontSize:'22px',fontWeight:700,color:GREEN}},
                (aiKeys.groqKeys||[]).filter(function(k){return k&&k.trim();}).length
              ),
              h('div',{style:{fontSize:'11px',color:MUTED}},'Groq Keys'),
              h('div',{style:{fontSize:'10px',color:GREEN}},
                (aiKeys.groqKeys||[]).filter(function(k){return k&&k.trim();}).length>0
                  ? '~'+((aiKeys.groqKeys||[]).filter(function(k){return k&&k.trim();}).length*200)+' req/day'
                  : 'No keys added'
              )
            ),
            h('div',{style:{background:BG,borderRadius:'10px',padding:'10px',textAlign:'center'}},
              h('div',{style:{fontSize:'22px',fontWeight:700,color:BLUE}},
                (aiKeys.cerebrasKeys||[]).filter(function(k){return k&&k.trim();}).length
              ),
              h('div',{style:{fontSize:'11px',color:MUTED}},'Cerebras Keys'),
              h('div',{style:{fontSize:'10px',color:BLUE}},
                (aiKeys.cerebrasKeys||[]).filter(function(k){return k&&k.trim();}).length>0
                  ? '~'+((aiKeys.cerebrasKeys||[]).filter(function(k){return k&&k.trim();}).length*200)+' req/day'
                  : 'No keys added'
              )
            )
          ),
          h('div',{style:{marginTop:'8px',fontSize:'11px',color:MUTED,textAlign:'center'}},
            'Total fast capacity: ~'+(
              ((aiKeys.groqKeys||[]).filter(function(k){return k&&k.trim();}).length +
               (aiKeys.cerebrasKeys||[]).filter(function(k){return k&&k.trim();}).length)*200
            )+' requests/day'
          )
        ),
            ...
            : allUsers.slice(0,5).map(function(u,i){
                return h('div',{key:u.uid||i,style:{display:'flex',gap:'10px',alignItems:'center',padding:'9px 0',borderBottom:i<4?'1px solid rgba(255,255,255,0.04)':'none'}},
                  ...
                  h('div',{style:{flex:1,minWidth:0}},
                    ...
                    h('div',{style:{fontSize:'10px',color:MUTED}},(u.method==='guest'?'Guest':'Ukey')+' · '+(u.lastLogin?new Date(u.lastLogin).toLocaleDateString():'Unknown'))
                  ),
      ...
      tab==='users' && h('div',null,
        h('div',{style:{display:'flex',gap:'6px',marginBottom:'12px',flexWrap:'wrap'}},
          ['all','active','blocked','guest','ukey'].map(function(f){
            return h('button',{key:f,onClick:function(){setUserFilter(f);},
              ...
              f==='all'?'All Users':f==='active'?'Active':f==='blocked'?'🚫 Blocked':f==='guest'?'👤 Guest':'🔑 Ukey'
            );
          })
        ),
                ...
                h('div',{style:{display:'flex',gap:'10px',alignItems:'flex-start'}},
                  ...
                  h('div',{style:{flex:1,minWidth:0}},
                    h('div',{style:{display:'flex',alignItems:'center',gap:'6px',marginBottom:'4px',flexWrap:'wrap'}},
                      ...
                      u.blocked && h('span',{style:badge(RED)},'🚫 Blocked'),
                      h('span',{style:badge(u.method==='guest'?BLUE:PURPLE)},u.method==='guest'?'👤 Google':'🔑 Ukey')
                    ),
      ...
      tab==='aikeys' && h('div',null,
        ...
        // ── MODEL CONFIGURATOR ──
        h('div',{style:{background:CARD,border:'1px solid '+BORDER,borderRadius:'16px',padding:'16px',marginBottom:'14px'}},
          h('div',{style:{fontWeight:700,fontSize:'14px',color:TEXT,marginBottom:'4px'}},'🤖 AI Model Configuration'),
          h('div',{style:{fontSize:'12px',color:MUTED,marginBottom:'14px'}},'Choose which model runs in each mode. Changes save automatically.'),

          // Speed comparison bar
          h('div',{style:{background:BG,borderRadius:'12px',padding:'12px',marginBottom:'14px'}},
            h('div',{style:{fontSize:'12px',fontWeight:700,color:MUTED,marginBottom:'10px',letterSpacing:'0.5px'}},'⚡ SPEED COMPARISON (tokens/second)'),
            AI_MODELS.map(function(m){
              var pct = Math.min(100, Math.round(m.speed/2600*100));
              return h('div',{key:m.id,style:{marginBottom:'8px'}},
                h('div',{style:{display:'flex',justifyContent:'space-between',marginBottom:'3px'}},
                  h('span',{style:{fontSize:'11px',color:TEXT,fontWeight:500}},m.name),
                  h('span',{style:{fontSize:'11px',color:m.color,fontWeight:700}},m.speedLabel)
                ),
                h('div',{style:{height:'6px',background:BORDER,borderRadius:'3px',overflow:'hidden'}},
                  h('div',{style:{height:'100%',width:pct+'%',background:m.color,borderRadius:'3px'}})
                )
              );
            })
          ),
          ...
          h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'14px'}},
            // SLOW mode selector
            h('div',null,
              h('div',{style:{fontSize:'12px',fontWeight:700,color:TEXT,marginBottom:'8px'}},'🐢 SLOW Mode Model'),
              h('div',{style:{display:'flex',flexDirection:'column',gap:'6px'}},
                AI_MODELS.map(function(m){
                  var active = modelConfig.slowModel===m.id;
                  return h('button',{key:m.id,
                    onClick:function(){
                      var nc=Object.assign({},modelConfig,{slowModel:m.id});
                      setModelConfig(nc);
                      try{localStorage.setItem('sh_fizux:modelconfig',JSON.stringify(nc));}catch(e){}
                      notify('✅ SLOW: '+m.name);
                    },
                    style:{
                      padding:'8px 10px',borderRadius:'10px',border:'2px solid '+(active?m.color:BORDER),
                      background:active?m.color+'18':BG,cursor:'pointer',textAlign:'left',
                      fontFamily:'inherit',transition:'all 0.2s'
                    }},
                    h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center'}},
                      h('div',null,
                        h('div',{style:{fontSize:'11px',fontWeight:700,color:active?m.color:TEXT}},m.name.split('—')[1]||m.name),
                        h('div',{style:{fontSize:'10px',color:MUTED}},m.speedLabel+' · '+m.limit)
                      ),
                      active && h('div',{style:{width:'8px',height:'8px',borderRadius:'50%',background:m.color}})
                    )
                  );
                })
              )
            ...
            // FAST mode selector
            h('div',null,
              h('div',{style:{fontSize:'12px',fontWeight:700,color:TEXT,marginBottom:'8px'}},'⚡ FAST Mode Model'),
              h('div',{style:{display:'flex',flexDirection:'column',gap:'6px'}},
                AI_MODELS.map(function(m){
                  var active = modelConfig.fastModel===m.id;
                  return h('button',{key:m.id,
                    onClick:function(){
                      var nc=Object.assign({},modelConfig,{fastModel:m.id});
                      setModelConfig(nc);
                      try{localStorage.setItem('sh_fizux:modelconfig',JSON.stringify(nc));}catch(e){}
                      notify('✅ FAST: '+m.name);
                    },
          ...
          // Same model both modes option
          h('div',{style:{background:BG,borderRadius:'10px',padding:'10px',marginBottom:'10px'}},
            h('div',{style:{fontSize:'12px',color:MUTED,marginBottom:'8px'}},'⚡ Quick — Same model for both modes:'),
            h('div',{style:{display:'flex',gap:'6px',flexWrap:'wrap'}},
              AI_MODELS.map(function(m){
                return h('button',{key:m.id,
                  onClick:function(){
                    var nc=Object.assign({},modelConfig,{slowModel:m.id,fastModel:m.id});
                    setModelConfig(nc);
                    try{localStorage.setItem('sh_fizux:modelconfig',JSON.stringify(nc));}catch(e){}
                    notify('✅ Both modes: '+m.name);
                  },
                  style:{padding:'5px 10px',borderRadius:'8px',border:'1px solid '+BORDER,
                    background:CARD,cursor:'pointer',fontSize:'11px',color:TEXT,fontFamily:'inherit'}},
          ...
          h('div',{style:{background:GREEN+'12',border:'1px solid '+GREEN+'30',borderRadius:'10px',padding:'10px'}},
            h('div',{style:{fontSize:'12px',fontWeight:700,color:TEXT,marginBottom:'6px'}},'📋 Current Config'),
            h('div',{style:{display:'flex',gap:'8px'}},
              h('div',{style:{flex:1,background:CARD,borderRadius:'8px',padding:'8px'}},
                h('div',{style:{fontSize:'10px',color:MUTED}},'🐢 SLOW'),
                h('div',{style:{fontSize:'11px',fontWeight:700,color:TEXT}},
                  (function(){var m=AI_MODELS.find(function(x){return x.id===modelConfig.slowModel;});return m?m.name.split('—')[1]:'Not set';})()
                )
              ),
              h('div',{style:{flex:1,background:CARD,borderRadius:'8px',padding:'8px'}},
                h('div',{style:{fontSize:'10px',color:MUTED}},'⚡ FAST'),
                h('div',{style:{fontSize:'11px',fontWeight:700,color:TEXT}},
                  (function(){var m=AI_MODELS.find(function(x){return x.id===modelConfig.fastModel;});return m?m.name.split('—')[1]:'Not set';})()
                )
              )
            )
          )
        ...
        // ── API KEYS ──
        h('div',{style:{fontSize:'13px',color:MUTED,marginBottom:'8px',fontWeight:700,letterSpacing:'0.5px'}},'🔑 API KEYS'),

        // Groq Keys
        h('div',{style:{background:CARD,border:'1px solid '+BORDER,borderRadius:'14px',padding:'14px',marginBottom:'10px'}},
          h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}},
            h('div',null,
              h('div',{style:{fontSize:'13px',fontWeight:700,color:TEXT}},'⚡ Groq Keys'),
              h('div',{style:{fontSize:'11px',color:MUTED}},'14,400 req/day per key')
            ),
            h('a',{href:'https://console.groq.com/keys',target:'_blank',style:{fontSize:'11px',color:GREEN,textDecoration:'none'}},
              'Get Key ↗')
          ),
          (aiKeys.groqKeys||['']).map(function(k,i){
            return h('div',{key:'gk'+i,style:{display:'flex',gap:'6px',marginBottom:'6px'}},
              h('input',{value:k,type:'password',
                onChange:function(e){var nk=Object.assign({},aiKeys);var arr=(nk.groqKeys||['']).slice();arr[i]=e.target.value;nk.groqKeys=arr;setAiKeys(nk);ST.set('fizux:aikeys',JSON.stringify(nk),true);},
                placeholder:'gsk_...',
              ...
              h('button',{onClick:function(){var nk=Object.assign({},aiKeys);var arr=(nk.groqKeys||['']).slice();arr.splice(i,1);if(arr.length===0)arr=[''];nk.groqKeys=arr;setAiKeys(nk);ST.set('fizux:aikeys',JSON.stringify(nk),true);notify('Removed');},
                style:{padding:'8px',borderRadius:'8px',border:'none',background:RED+'22',color:RED,cursor:'pointer',fontSize:'13px'}},'✕')
            );
          }),
          h('button',{onClick:function(){var nk=Object.assign({},aiKeys);nk.groqKeys=(nk.groqKeys||['']).concat(['']);setAiKeys(nk);},
            style:{width:'100%',padding:'8px',borderRadius:'8px',border:'1px dashed '+BORDER,background:'transparent',color:MUTED,cursor:'pointer',fontSize:'12px',fontFamily:'inherit'}},
            '+ Add Groq Key'),
          h('div',{style:{fontSize:'11px',color:GREEN,marginTop:'6px'}},
            (aiKeys.groqKeys||[]).filter(function(k){return k.trim();}).length+' key(s) · ~'+
            ((aiKeys.groqKeys||[]).filter(function(k){return k.trim();}).length*14400).toLocaleString()+' req/day')
        ),
        ...
        h('div',{style:{background:CARD,border:'1px solid '+BORDER,borderRadius:'14px',padding:'14px',marginBottom:'10px'}},
          h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}},
            h('div',null,
              h('div',{style:{fontSize:'13px',fontWeight:700,color:TEXT}},'⚡⚡ Cerebras Keys'),
              h('div',{style:{fontSize:'11px',color:MUTED}},'~2,000 req/day per key · Fastest AI')
            ),
            h('a',{href:'https://inference.cerebras.ai',target:'_blank',style:{fontSize:'11px',color:ORANGE,textDecoration:'none'}},
              'Get Key ↗')
          ),
          (aiKeys.cerebrasKeys||['']).map(function(k,i){
            return h('div',{key:'ck'+i,style:{display:'flex',gap:'6px',marginBottom:'6px'}},
              h('input',{value:k,type:'password',
                onChange:function(e){var nk=Object.assign({},aiKeys);var arr=(nk.cerebrasKeys||['']).slice();arr[i]=e.target.value;nk.cerebrasKeys=arr;setAiKeys(nk);ST.set('fizux:aikeys',JSON.stringify(nk),true);},
                placeholder:'csk_...',
                style:{flex:1,background:BG,border:'1px solid '+BORDER,borderRadius:'8px',padding:'8px 10px',color:TEXT,fontFamily:'monospace',fontSize:'12px',outline:'none'}}),
              h('button',{onClick:function(){var nk=Object.assign({},aiKeys);var arr=(nk.cerebrasKeys||['']).slice();arr.splice(i,1);if(arr.length===0)arr=[''];nk.cerebrasKeys=arr;setAiKeys(nk);ST.set('fizux:aikeys',JSON.stringify(nk),true);notify('Removed');},
                style:{padding:'8px',borderRadius:'8px',border:'none',background:RED+'22',color:RED,cursor:'pointer',fontSize:'13px'}},'✕')
            );
          }),
          h('button',{onClick:function(){var nk=Object.assign({},aiKeys);nk.cerebrasKeys=(nk.cerebrasKeys||['']).concat(['']);setAiKeys(nk);},
            style:{width:'100%',padding:'8px',borderRadius:'8px',border:'1px dashed '+BORDER,background:'transparent',color:MUTED,cursor:'pointer',fontSize:'12px',fontFamily:'inherit'}},
            '+ Add Cerebras Key'),
          h('div',{style:{fontSize:'11px',color:ORANGE,marginTop:'6px'}},
            (aiKeys.cerebrasKeys||[]).filter(function(k){return k.trim();}).length+' key(s) · ~'+
            ((aiKeys.cerebrasKeys||[]).filter(function(k){return k.trim();}).length*2000).toLocaleString()+' req/day')
        ),

        // Gemini Keys
        h('div',{style:{background:CARD,border:'1px solid '+BORDER,borderRadius:'14px',padding:'14px',marginBottom:'14px'}},
          h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}},
            h('div',null,
              h('div',{style:{fontSize:'13px',fontWeight:700,color:TEXT}},'✨ Gemini Keys'),
              h('div',{style:{fontSize:'11px',color:MUTED}},'1,000 req/day per key · Best free limit')
            ),
            h('a',{href:'https://aistudio.google.com/apikey',target:'_blank',style:{fontSize:'11px',color:BLUE,textDecoration:'none'}},
              'Get Key ↗')
          ),
          (aiKeys.geminiKeys||['']).map(function(k,i){
            return h('div',{key:'gem'+i,style:{display:'flex',gap:'6px',marginBottom:'6px'}},
              h('input',{value:k,type:'password',
                onChange:function(e){var nk=Object.assign({},aiKeys);var arr=(nk.geminiKeys||['']).slice();arr[i]=e.target.value;nk.geminiKeys=arr;setAiKeys(nk);ST.set('fizux:aikeys',JSON.stringify(nk),true);},
                placeholder:'AIza...',
                style:{flex:1,background:BG,border:'1px solid '+BORDER,borderRadius:'8px',padding:'8px 10px',color:TEXT,fontFamily:'monospace',fontSize:'12px',outline:'none'}}),
              h('button',{onClick:function(){var nk=Object.assign({},aiKeys);var arr=(nk.geminiKeys||['']).slice();arr.splice(i,1);if(arr.length===0)arr=[''];nk.geminiKeys=arr;setAiKeys(nk);ST.set('fizux:aikeys',JSON.stringify(nk),true);notify('Removed');},
                style:{padding:'8px',borderRadius:'8px',border:'none',background:RED+'22',color:RED,cursor:'pointer',fontSize:'13px'}},'✕')
            );
          }),
          h('button',{onClick:function(){var nk=Object.assign({},aiKeys);nk.geminiKeys=(nk.geminiKeys||['']).concat(['']);setAiKeys(nk);},
            style:{width:'100%',padding:'8px',borderRadius:'8px',border:'1px dashed '+BORDER,background:'transparent',color:MUTED,cursor:'pointer',fontSize:'12px',fontFamily:'inherit'}},
            '+ Add Gemini Key'),
          h('div',{style:{fontSize:'11px',color:BLUE,marginTop:'6px'}},
            (aiKeys.geminiKeys||[]).filter(function(k){return k.trim();}).length+' key(s) · '+
            ((aiKeys.geminiKeys||[]).filter(function(k){return k.trim();}).length*1000).toLocaleString()+' req/day')
        ),

        // ── ElevenLabs Key ──
        h('div',{style:{background:CARD,border:'1px solid rgba(139,92,246,0.3)',borderRadius:'14px',padding:'14px',marginBottom:'14px'}},
          h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}},
            h('div',null,
              h('div',{style:{fontSize:'13px',fontWeight:700,color:TEXT}},'✨ ElevenLabs Key'),
              h('div',{style:{fontSize:'11px',color:MUTED}},'Real human TTS · 10,000 chars/month free')
            ),
            h('a',{href:'https://elevenlabs.io',target:'_blank',style:{fontSize:'11px',color:PURPLE,textDecoration:'none'}},
              'Get Key ↗')
          ),
          h('input',{
            value:aiKeys.elevenKey||'',
            type:'password',
            onChange:function(e){
              var nk=Object.assign({},aiKeys,{elevenKey:e.target.value});
              setAiKeys(nk);
              // Save to shared storage separately
              try{localStorage.setItem('sh_fizux:elevenkey',e.target.value);}catch(err){}
            },
            placeholder:'sk_...',
            style:{width:'100%',background:BG,border:'1px solid '+BORDER,borderRadius:'8px',
              padding:'8px 10px',color:TEXT,fontFamily:'monospace',fontSize:'12px',outline:'none',
              boxSizing:'border-box'}
          }),
          (aiKeys.elevenKey||'').length>10 && h('div',{style:{fontSize:'11px',color:GREEN,marginTop:'6px'}},
            '✅ Real human voice enabled for all users'
          )
        ),

        h('button',{onClick:function(){
          ST.set('fizux:aikeys',JSON.stringify(aiKeys),true).then(function(){notify('✅ All Keys Saved!');}).catch(function(){notify('❌ Save failed');});
          try{localStorage.setItem('sh_fizux:modelconfig',JSON.stringify(modelConfig));}catch(e){}
          if(aiKeys.elevenKey) try{localStorage.setItem('sh_fizux:elevenkey',aiKeys.elevenKey);}catch(e){}
        },style:{width:'100%',padding:'12px',background:GOLD,color:'#FFFFFF',border:'none',borderRadius:'12px',cursor:'pointer',fontWeight:700,fontSize:'14px',fontFamily:'inherit'}},'💾 Save All')
      ),
