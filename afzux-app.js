...
function AFZUXApp(){
  ...
  var s60=useState({groqKeys:[''],orKeys:[''],hfToken:'',geminiKey:'',claudeKey:'',openaiKey:'',grokKey:''}); var aiKeys=s60[0]; var setAiKeys=s60[1];
  // Generate tab
    ...
    h('div',{style:{padding:'14px',maxWidth:'760px',margin:'0 auto'}},
      ...
      tab==='overview' && h('div',null,
        ...
        h('div',{style:card({marginBottom:'12px'})},
          ...
          h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}},
            ...
            h('div',{style:{background:BG,borderRadius:'10px',padding:'10px',textAlign:'center'}},
              h('div',{style:{fontSize:'22px',fontWeight:700,color:BLUE}},
                (aiKeys.orKeys||[]).filter(function(k){return k&&k.trim();}).length
              ),
              h('div',{style:{fontSize:'11px',color:MUTED}},'OpenRouter Keys'),
              h('div',{style:{fontSize:'10px',color:BLUE}},
                (aiKeys.orKeys||[]).filter(function(k){return k&&k.trim();}).length>0
                  ? '~'+((aiKeys.orKeys||[]).filter(function(k){return k&&k.trim();}).length*200)+' req/day'
                  : 'No keys added'
              )
            )
          ),
          h('div',{style:{marginTop:'8px',fontSize:'11px',color:MUTED,textAlign:'center'}},
            'Total fast capacity: ~'+(
              ((aiKeys.groqKeys||[]).filter(function(k){return k&&k.trim();}).length +
               (aiKeys.orKeys||[]).filter(function(k){return k&&k.trim();}).length)*200
            )+' requests/day'
          )
        ),
      ...
      tab==='aikeys' && h('div',null,
        ...
        h('div',{style:{background:CARD,border:'1px solid '+BORDER,borderRadius:'14px',padding:'16px',marginBottom:'14px'}},
          h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}},
            h('div',null,
              ...
              h('div',{style:{fontSize:'11px',color:MUTED}},'200 req/day per key - fastest AI')
            ),
        ...
        h('div',{style:{background:CARD,border:'1px solid '+BORDER,borderRadius:'14px',padding:'16px',marginBottom:'14px'}},
          h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}},
            h('div',null,
              h('div',{style:{fontSize:'13px',fontWeight:700,color:TEXT}},'\u25c8 OpenRouter Keys'),
              h('div',{style:{fontSize:'11px',color:MUTED}},'200 req/day per key - 29+ free models')
            ),
            h('a',{href:'https://openrouter.ai/keys',target:'_blank',style:{fontSize:'11px',color:GOLD,textDecoration:'none'}},'Get Key \u2197')
          ),
          (aiKeys.orKeys||['']).map(function(k,i){
            return h('div',{key:'ok'+i,style:{display:'flex',gap:'8px',marginBottom:'8px'}},
              h('input',{type:'password',value:k,
                onChange:function(e){var nk=Object.assign({},aiKeys);var arr=(nk.orKeys||['']).slice();arr[i]=e.target.value;nk.orKeys=arr;setAiKeys(nk);ST.set('fizux:aikeys',JSON.stringify(nk),true);},
                placeholder:'Enter OpenRouter key...',
                style:{flex:1,background:BG,border:'1px solid '+BORDER,borderRadius:'8px',padding:'8px 10px',color:TEXT,fontFamily:'monospace',fontSize:'12px',outline:'none'}}),
              h('button',{onClick:function(){var nk=Object.assign({},aiKeys);var arr=(nk.orKeys||['']).slice();arr.splice(i,1);if(arr.length===0)arr=[''];nk.orKeys=arr;setAiKeys(nk);ST.set('fizux:aikeys',JSON.stringify(nk),true);notify('Removed!');},
                style:{padding:'8px 10px',background:'rgba(255,77,106,0.1)',color:RED,border:'none',borderRadius:'8px',cursor:'pointer',fontSize:'13px'}},'\u2715')
            );
          }),
          h('button',{onClick:function(){var nk=Object.assign({},aiKeys);nk.orKeys=(nk.orKeys||['']).concat(['']);setAiKeys(nk);},
            style:{width:'100%',padding:'8px',background:'rgba(201,168,76,0.08)',border:'1px dashed '+GOLD,borderRadius:'8px',color:GOLD,cursor:'pointer',fontSize:'12px',fontFamily:'inherit',marginBottom:'10px'}},'+ Add OpenRouter Key'),
          h('div',{style:{fontSize:'11px',color:GREEN}},(function(){
                var cnt=(aiKeys.orKeys||[]).filter(function(k){return k.trim();}).length;
        ...
        h('div',{style:{background:'rgba(46,204,138,0.06)',border:'1px solid rgba(46,204,138,0.2)',borderRadius:'10px',padding:'10px 14px',marginBottom:'14px'}},
          ...
          h('div',{style:{fontSize:'11px',color:MUTED}},'Groq \u2192 OpenRouter \u2192 Pollinations (unlimited fallback)')
        ),
