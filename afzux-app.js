...
function AFZUXApp(){
  ...
  var s60=useState({groqKeys:[''],cerebrasKeys:[''],hfToken:'',geminiKey:'',claudeKey:'',openaiKey:'',grokKey:''}); var aiKeys=s60[0]; var setAiKeys=s60[1];
  // Generate tab
  ...
  return h('div',{style:{minHeight:'100vh',background:BG,color:TEXT}},
    ...
    h('div',{style:{padding:'14px',maxWidth:'760px',margin:'0 auto'}},
      ...
      tab==='aikeys' && h('div',null,
        h('div',{style:{fontSize:'13px',color:MUTED,marginBottom:'16px'}},'Add multiple keys per provider. FIZUX rotates them automatically.'),

        h('div',{style:{background:CARD,border:'1px solid '+BORDER,borderRadius:'14px',padding:'16px',marginBottom:'14px'}},
          h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}},
            h('div',null,
              h('div',{style:{fontSize:'13px',fontWeight:700,color:TEXT}},'\u26a1 Groq Keys'),
              h('div',{style:{fontSize:'11px',color:MUTED}},'~1300 req/day per key - fastest AI')
            ),
            h('a',{href:'https://console.groq.com/keys',target:'_blank',style:{fontSize:'11px',color:GOLD,textDecoration:'none'}},'Get Key \u2197')
          ),
          (aiKeys.groqKeys||['']).map(function(k,i){
            return h('div',{key:'gk'+i,style:{display:'flex',gap:'8px',marginBottom:'8px'}},
              h('input',{type:'password',value:k,
              ...
              h('button',{onClick:function(){var nk=Object.assign({},aiKeys);var arr=(nk.groqKeys||['']).slice();arr.splice(i,1);if(arr.length===0)arr=[''];nk.groqKeys=arr;setAiKeys(nk);ST.set('fizux:aikeys',JSON.stringify(nk),true);notify('Removed!');},
                style:{padding:'8px 10px',background:'rgba(255,77,106,0.1)',color:RED,border:'none',borderRadius:'8px',cursor:'pointer',fontSize:'13px'}},'\u2715')
            );
          }),
          h('button',{onClick:function(){var nk=Object.assign({},aiKeys);nk.groqKeys=(nk.groqKeys||['']).concat(['']);setAiKeys(nk);},
            style:{width:'100%',padding:'8px',background:'rgba(201,168,76,0.08)',border:'1px dashed '+GOLD,borderRadius:'8px',color:GOLD,cursor:'pointer',fontSize:'12px',fontFamily:'inherit',marginBottom:'10px'}},'+ Add Groq Key'),
          h('div',{style:{fontSize:'11px',color:GREEN}},(function(){
                var cnt=(aiKeys.groqKeys||[]).filter(function(k){return k.trim();}).length;
                return cnt>0?'✅ '+cnt+' Groq key'+(cnt>1?'s':'')+'  •  ~'+(cnt*200)+' req/day':'⚠️ No keys added';
              })())
        ),
        ...
        h('div',{style:{background:CARD,border:'1px solid '+BORDER,borderRadius:'14px',padding:'16px',marginBottom:'14px'}},
          h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}},
            h('div',null,
              ...
              h('div',{style:{fontSize:'11px',color:MUTED}},'~1300 req/day per key - Llama 3.3 70B')
            ),
            h('a',{href:'https://inference.cerebras.ai',target:'_blank',style:{fontSize:'11px',color:GOLD,textDecoration:'none'}},'Get Key \u2197')
          ),
          (aiKeys.cerebrasKeys||['']).map(function(k,i){
            return h('div',{key:'ok'+i,style:{display:'flex',gap:'8px',marginBottom:'8px'}},
              h('input',{type:'password',value:k,
          ...
          h('button',{onClick:function(){var nk=Object.assign({},aiKeys);nk.cerebrasKeys=(nk.cerebrasKeys||['']).concat(['']);setAiKeys(nk);},
            style:{width:'100%',padding:'8px',background:'rgba(201,168,76,0.08)',border:'1px dashed '+GOLD,borderRadius:'8px',color:GOLD,cursor:'pointer',fontSize:'12px',fontFamily:'inherit',marginBottom:'10px'}},'+ Add Cerebras Key'),
          h('div',{style:{fontSize:'11px',color:GREEN}},(function(){
                var cnt=(aiKeys.cerebrasKeys||[]).filter(function(k){return k.trim();}).length;
                return cnt>0?'✅ '+cnt+' OR key'+(cnt>1?'s':'')+'  •  ~'+(cnt*200)+' req/day':'⚠️ No keys added';
              })())
        ),

        h('div',{style:{background:'rgba(46,204,138,0.06)',border:'1px solid rgba(46,204,138,0.2)',borderRadius:'10px',padding:'10px 14px',marginBottom:'14px'}},
          h('div',{style:{fontSize:'11px',color:GREEN,fontWeight:600,marginBottom:'4px'}},'\ud83d\udd04 Auto Rotation'),
          h('div',{style:{fontSize:'11px',color:MUTED}},'Cerebras (FAST) \u2192 Groq (SLOW)')
        ),
        ...
        },style:{width:'100%',padding:'12px',background:GOLD,color:'#FFFFFF',border:'none',borderRadius:'12px',cursor:'pointer',fontWeight:700,fontSize:'14px',fontFamily:'inherit'}},'\ud83d\udcbe Save All Keys'),

        // ── API Usage & Remaining ──
        h('div',{style:{background:CARD,border:'1px solid '+BORDER,borderRadius:'14px',padding:'16px',marginTop:'14px'}},
          h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'14px'}},
            h('div',{style:{fontWeight:700,fontSize:'13px',color:TEXT}},'\ud83d\udcca Today\'s Usage & Remaining'),
            h('button',{onClick:loadUsage,style:{fontSize:'12px',color:BLUE,background:'none',border:'none',cursor:'pointer',fontFamily:'inherit'}},
              '\ud83d\udd04 Refresh')
          ),
          h('div',{style:{fontSize:'11px',color:MUTED,marginBottom:'12px'}},
            'Date: '+usage.date
          ),

          // Groq row
          (function(){
            var groqKeys = (aiKeys.groqKeys||[]).filter(function(k){return k&&k.trim();}).length;
            var groqCap = groqKeys * 14400;
            var groqUsed = usage.groqUsed || 0;
            var groqLeft = Math.max(0, groqCap - groqUsed);
            var groqPct = groqCap > 0 ? Math.min(100, Math.round(groqUsed/groqCap*100)) : 0;
            var groqColor = groqPct > 80 ? RED : groqPct > 50 ? ORANGE : GREEN;
            return h('div',{style:{background:BG,borderRadius:'10px',padding:'12px',marginBottom:'10px'}},
              h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}},
                h('div',{style:{fontWeight:600,fontSize:'13px',color:TEXT}},'\u26a1 Groq — SLOW Mode'),
                h('a',{href:'https://console.groq.com/dashboard',target:'_blank',
                  style:{fontSize:'11px',color:BLUE,textDecoration:'none'}},
                  'Dashboard \u2197')
              ),
              h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px',marginBottom:'10px'}},
                h('div',{style:{textAlign:'center',background:CARD,borderRadius:'8px',padding:'8px'}},
                  h('div',{style:{fontSize:'20px',fontWeight:800,color:BLUE}},groqCap.toLocaleString()),
                  h('div',{style:{fontSize:'11px',color:MUTED}},'Total/Day')
                ),
                h('div',{style:{textAlign:'center',background:CARD,borderRadius:'8px',padding:'8px'}},
                  h('div',{style:{fontSize:'20px',fontWeight:800,color:groqColor}},groqUsed.toLocaleString()),
                  h('div',{style:{fontSize:'11px',color:MUTED}},'Used Today')
                ),
                h('div',{style:{textAlign:'center',background:CARD,borderRadius:'8px',padding:'8px'}},
                  h('div',{style:{fontSize:'20px',fontWeight:800,color:GREEN}},groqLeft.toLocaleString()),
                  h('div',{style:{fontSize:'11px',color:MUTED}},'Remaining')
                )
              ),
              h('div',{style:{height:'6px',background:BORDER,borderRadius:'3px',overflow:'hidden'}},
                h('div',{style:{height:'100%',width:groqPct+'%',background:groqColor,borderRadius:'3px',transition:'width 0.3s'}})
              ),
              h('div',{style:{display:'flex',justifyContent:'space-between',fontSize:'11px',color:MUTED,marginTop:'4px'}},
                h('span',null,groqPct+'% used'),
                h('span',null,groqKeys+' key'+(groqKeys!==1?'s':'')+' × 14,400')
              )
            );
          })(),
          ...
          // Cerebras row
          (function(){
            var cKeys = (aiKeys.cerebrasKeys||[]).filter(function(k){return k&&k.trim();}).length;
            var cCap = cKeys * 1300;
            var cUsed = usage.cerebrasUsed || 0;
            var cLeft = Math.max(0, cCap - cUsed);
            var cPct = cCap > 0 ? Math.min(100, Math.round(cUsed/cCap*100)) : 0;
            var cColor = cPct > 80 ? RED : cPct > 50 ? ORANGE : GREEN;
            return h('div',{style:{background:BG,borderRadius:'10px',padding:'12px',marginBottom:'10px'}},
              h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}},
                h('div',{style:{fontWeight:600,fontSize:'13px',color:TEXT}},'\u26a1\u26a1 Cerebras — FAST Mode'),
                h('a',{href:'https://inference.cerebras.ai',target:'_blank',
                  style:{fontSize:'11px',color:ORANGE,textDecoration:'none'}},
                  'Dashboard \u2197')
              ),
              h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px',marginBottom:'10px'}},
                h('div',{style:{textAlign:'center',background:CARD,borderRadius:'8px',padding:'8px'}},
                  h('div',{style:{fontSize:'20px',fontWeight:800,color:ORANGE}},cCap.toLocaleString()),
                  h('div',{style:{fontSize:'11px',color:MUTED}},'Total/Day')
                ),
                h('div',{style:{textAlign:'center',background:CARD,borderRadius:'8px',padding:'8px'}},
                  h('div',{style:{fontSize:'20px',fontWeight:800,color:cColor}},cUsed.toLocaleString()),
                  h('div',{style:{fontSize:'11px',color:MUTED}},'Used Today')
                ),
                h('div',{style:{textAlign:'center',background:CARD,borderRadius:'8px',padding:'8px'}},
                  h('div',{style:{fontSize:'20px',fontWeight:800,color:GREEN}},cLeft.toLocaleString()),
                  h('div',{style:{fontSize:'11px',color:MUTED}},'Remaining')
                )
              ),
              h('div',{style:{height:'6px',background:BORDER,borderRadius:'3px',overflow:'hidden'}},
                h('div',{style:{height:'100%',width:cPct+'%',background:cColor,borderRadius:'3px',transition:'width 0.3s'}})
              ),
              h('div',{style:{display:'flex',justifyContent:'space-between',fontSize:'11px',color:MUTED,marginTop:'4px'}},
                h('span',null,cPct+'% used'),
                h('span',null,cKeys+' key'+(cKeys!==1?'s':'')+' × 1,300')
              )
            );
          })(),

          // Total summary
          h('div',{style:{background:GREEN+'12',border:'1px solid '+GREEN+'30',borderRadius:'10px',padding:'12px'}},
            h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center'}},
              h('div',null,
                h('div',{style:{fontSize:'13px',fontWeight:700,color:TEXT}},'\ud83d\udcca Combined Today'),
                h('div',{style:{fontSize:'11px',color:MUTED,marginTop:'2px'}},'Groq + Cerebras total')
              ),
              h('div',{style:{textAlign:'right'}},
                h('div',{style:{fontSize:'22px',fontWeight:900,color:GREEN}},
                  (Math.max(0,
                    (aiKeys.groqKeys||[]).filter(function(k){return k&&k.trim();}).length * 14400 - (usage.groqUsed||0) +
                    (aiKeys.cerebrasKeys||[]).filter(function(k){return k&&k.trim();}).length * 1300 - (usage.cerebrasUsed||0)
                  )).toLocaleString()
                ),
                h('div',{style:{fontSize:'11px',color:MUTED}},'requests remaining')
              )
            )
          ),
          h('div',{style:{fontSize:'11px',color:MUTED,marginTop:'8px',textAlign:'center'}},
            '\u26a0\ufe0f Estimated based on app usage. Click Refresh to update.'
          )
        )
      ),
